...
void KSignalPlotter::setShowAxis( bool value )
{
  ...
  if( mShowAxis ) {
    if ( kapp->layoutDirection() == Qt::RightToLeft )
      mPlottingArea.setRight(mPlottingArea.right() - mAxisTextWidth - 10);
    else
      mPlottingArea.setLeft(mAxisTextWidth+10);
  }
  update();
}
...
void KSignalPlotter::updateDataBuffers()
{
  ...
  /* Need to account for axis text width, or there may be a long delay before
   * a rescale happens */
  int axisWidth = 0;
  if ( mShowAxis )
    axisWidth = mAxisTextWidth + 10;

  mSamples = static_cast<uint>( ( ( width() - axisWidth - 2 ) /
